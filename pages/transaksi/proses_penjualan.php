...
$sql_cek_cart = $connect->query("SELECT * FROM keranjang where id_barang = '$id_barang' && id_user = '$id_user' && status_keranjang=1")->rowCount();
...
function(isConfirm){
if (isConfirm) {
window.location.href = "../../index.php?page=penjualan&action=add-penjualan";
}
}); });
</script>
<!--  -->
...
try {
// query insert data
$sql = "INSERT into keranjang values ('$id','$id_user','$id_barang','$harga_jual','$qty','$subtotal', '$status' )";
// menyiapkan query
...
$pembayaran = str_replace(".", "", $_POST['harga']);
...
if ($pembayaran < $total_belanja) { ?>
    ...
    swal({
    title: "Oops...",
    text: "Jumlah pembayaran kurang, silahkan cek kembali !",
    type: "error",
    confirmButtonText: "OK"
    },
    function(isConfirm){
...
try {
  ...
  $sql_produk = $connect->query("SELECT * FROM keranjang where id_user='$id_user' and status_keranjang='1'");
  while ($data_produk = $sql_produk->fetch(PDO::FETCH_OBJ)) {

    $sql_id = "SELECT * FROM detail_penjualan order by id_det_jual desc limit 1";
    $query_id = $connect->query($sql_id)->rowCount();
    if ($query_id > 0) {
    $result = $connect->query($sql_id);
    $data_id = $result->fetch(PDO::FETCH_OBJ);
    $id_det = $data_id->id_det_jual+1;
    }else{
    $id_det = 1;
    }
  ...
  $sql_detail = "INSERT into detail_penjualan values ('$id_det','$id', '$id_barang','$harga_produk','$qty_produk','$subtotal')";
    // menyiapkan query
...
// query insert data
$sql = "INSERT into penjualan values ('$id', '$tanggal','$id_user','$total_qty','$total_belanja','$pembayaran','$kembalian')";
// menyiapkan query
$query = $connect->prepare($sql); 
// eksekusi query
$query->execute();
...
if (isConfirm) {
window.location.href = "../../index.php?page=penjualan&action=add-penjualan";
window.open("../laporan/nota_penjualan.php?nota=<?php echo $id ?>","_blank");
}
}); });
</script>
<!--  -->
...
<!-- HAPUS SEMUA KERANJANG BELANJA / BATAL TRANSAKSI  -->
...
// query update data
$sql = "DELETE FROM keranjang where id_user = '$id_user' and status_keranjang='1'";
// menyiapkan query
$query = $connect->prepare($sql);
//mengeksekusi query
$query->execute();
